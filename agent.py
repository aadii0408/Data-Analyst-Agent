...
def PlotCodeGeneratorTool(cols: List[str], query: str) -> str:
    ...
    Rules
    -----as plt) for plotting.
    2. Assign the final result (Data
    1. Use pandas for data manipulation and matplotlib.pyplot (Frame, Series, scalar *or* matplotlib Figure) to a variable named `result`.
...
def CodeGenerationAgent(query: str, df: pd.DataFrame):
    ...
    prompt = (
        PlotCodeGeneratorTool(df.columns.tolist(), query)
        if should_plot
        else CodeWritingTool(df.columns.tolist(), query)
    )
    ...
    for chunk in response:
        if chunk.choices[0].delta.content is not None:
            ...
            if "</think>" in token:
                ...
                in_think = False
            if in_think or (
                "<think>" in full_response and not "</think>" in full_response
            ):
...
def main():
    ...
    with left:
        ...
        st.markdown(
            "<medium>Powered by <a href='https://build.nvidia.com/nvidia/llama-3_1-nemotron-ultra-253b-v1'>NVIDIA Llama-3.1-Nemotron-Ultra-253B-v1</a></medium>",
            unsafe_allow_html=True,
        )
        file = st.file_uploader("Choose CSV", type=["csv"])
        if file:
            if ("df" not in st.session_state) or (
                st.session_state.get("current_file") != file.name
            ):
                ...
                with st.chat_message(msg["role"]):
                    ...
                    if msg.get("plot_index") is not None:
                        ...
                        if 0 <= idx < len(st.session_state.plots):
                            ...
                            st.pyplot(
                                st.session_state.plots[idx], use_container_width=False
                            )
            ...
            if user_q := st.chat_input("Ask about your data…"):
                ...
                with st.spinner("Working …"):
                    code, should_plot_flag, code_thinking = CodeGenerationAgent(
                        user_q, st.session_state.df
                    )
                    result_obj = ExecutionAgent(
                        code, st.session_state.df, should_plot_flag
                    )
                ...
                if is_plot:
                    fig = (
                        result_obj.figure
                        if isinstance(result_obj, plt.Axes)
                        else result_obj
                    )
